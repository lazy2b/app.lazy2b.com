			...
			if (fo == null) {
				...
				try {
					if (fo instanceof Integer) {
						...
						tmpFieldValStr = f.getName() + "=" + (String) f.get(this);
					} else if (fo instanceof Double) {
						tmpFieldValStr = f.getName() + "=" + new DecimalFormat(".##").format(f.getDouble(this));
						...
						tmpFieldValStr = f.getName() + "=" + ((HashMap<?, ?>) f.get(this)).toString();
					} else if (fo instanceof Object) {
						tmpFieldValStr = f.getName() + "=" + f.get(this).toString();
					}
				} catch (Exception e) {
